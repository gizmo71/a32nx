...
void AutopilotStateMachineModelClass::step()
{
  ...
  if (AutopilotStateMachine_U.in.data.nav_valid && AutopilotStateMachine_U.in.data.nav_loc_valid) {
    ...
    if (a < 115.0) {
      ...
      if (AutopilotStateMachine_U.in.input.Phi_loc_c < 0.0) {
        ...
        L = AutopilotStateMachine_U.in.input.Phi_loc_c;
      }

      u = std::abs(AutopilotStateMachine_U.in.input.Phi_loc_c);
      if (((a > 25.0) && ((b_L < 10.0) && ((R == L) && (u > 5.0)))) || (b_L < 1.92)) {
        if ((R == L) && (u > 5.0)) {
          AutopilotStateMachine_B.BusAssignment_g.lateral.condition.LOC_CPT = true;
        } else {
          AutopilotStateMachine_B.BusAssignment_g.lateral.condition.LOC_CPT = ((a < 15.0) && (b_L < 1.1));
        }
      } else {
  ...
  while (high_i > low_ip1) {
    ...
    if (AutopilotStateMachine_U.in.data.H_dot_ft_min >= b[mid_i - 1]) {
      low_i = mid_i - 1;
      low_ip1 = mid_i + 1;
    } else {
      high_i = mid_i;
  ...
  b_L = AutopilotStateMachine_U.in.data.H_dot_ft_min - static_cast<real_T>(b[low_i]);
  a = AutopilotStateMachine_U.in.data.H_dot_ft_min * 0.00508;
  a = a * a / ((((b_L * c[low_i] + c[low_i + 6]) * b_L + c[low_i + 12]) * b_L + c[low_i + 18]) * 9.81) *
    3.2808398950131235;
  if (80.0 > a) {
    a = 80.0;
  }
  ...
  if ((AutopilotStateMachine_U.in.input.H_constraint_ft != 0.0) && (AutopilotStateMachine_U.in.input.H_constraint_ft !=
    ...
    if (std::abs(u) <= a / 5.0) {
      ...
      if (AutopilotStateMachine_U.in.data.H_dot_ft_min < 0.0) {
        ...
        b_L = AutopilotStateMachine_U.in.data.H_dot_ft_min;
      }

      AutopilotStateMachine_B.BusAssignment_g.vertical.condition.ALT_CST_CPT = ((u == b_L) && ((a >= 300.0) &&
